...
//    int right = 2 * node + 2;
//    if(left < size){
//        printHeap(heap,size,left,d+1);
//    }
//    if(right < size){
//        printHeap(heap,size,right,d+1);
//    }
...
void remove(int* heap, int key, int& size) { // O(n)
    for(int i=0; i<size; i++){
        if(heap[i] == key){
            heap[i] = heap[size-1];
            size--;
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) { // O(n^2)
    int* returnHeap = new int[size1+size2];
    for(int i=0; i<size1; i++){
        returnHeap[i] = heap1[i];
    }
    for(int j=size1; j<size1+size2; j++){
        for(int k=0; k<size2; k++){
            returnHeap[j] = heap2[k];
        }
    }
    heapify(returnHeap,size1+size2);
    return returnHeap;
}
